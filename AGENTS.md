...
- Preserve a stable camera format and continuous sample timestamps across selection, replay, pause, and resume. Choose the format from the highest-resolution supported fixture that can be decoded at startup so a small initial source does not reduce later video quality. Failed source preparation must leave the active source intact.
